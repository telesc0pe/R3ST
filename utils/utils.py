...
def save_checkpoint(model_path, epoch, iteration, model, optimizer,mae_loss, OnlyOne=True):
    state = {
        'epoch': epoch,
        'iter': iteration,
    ...
    model_filenames = [filename for filename in os.listdir(model_path) if filename.endswith('.pth')]
    if OnlyOne:
        for filename in model_filenames:
            os.remove(model_path+'/'+filename)

    torch.save(state, os.path.join(model_path, f'net_{epoch}epoch_mae_{mae_loss*255:.3f}.pth'))
